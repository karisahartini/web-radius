...
if(isset($_GET['hapus'])){
    ...
    if($hapus && mysqli_affected_rows($conn) > 0){
        ...
        $msg = "Gagal menghapus user. Error: " . mysqli_error($conn);
        $msgType = "error";
    }
}
...
body { background: var(--bg); color: var(--text); font-family: var(--font-body); padding: 32px 36px 60px; min-height: 100vh; }
.page-header { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 28px; }
.page-title { font-size: 22px; font-weight: 600; }
.page-sub { font-size: 13px; color: var(--muted); margin-top: 3px; }
.total-badge { font-family: var(--font-mono); font-size: 12px; padding: 5px 14px; border-radius: 20px; background: rgba(59,130,246,.1); border: 1px solid rgba(59,130,246,.25); color: var(--accent); }
.alert { display: flex; align-items: center; gap: 10px; padding: 13px 18px; border-radius: 10px; font-size: 14px; margin-bottom: 22px; animation: fadeIn .3s ease; }
...
.card-header { padding: 16px 22px; border-bottom: 1px solid var(--card-border); display: flex; align-items: center; gap: 9px; font-size: 14px; font-weight: 600; }
.card-header i { color: var(--accent); font-size: 16px; }
.card-body { padding: 22px; }
.form-group { display: flex; flex-direction: column; gap: 6px; margin-bottom: 16px; }
.form-label { font-size: 12px; letter-spacing: 1px; text-transform: uppercase; color: var(--muted); font-weight: 500; }
.form-input { background: rgba(255,255,255,.05); border: 1px solid var(--card-border); border-radius: 9px; padding: 11px 14px; color: var(--text); font-family: var(--font-body); font-size: 14px; width: 100%; transition: border-color .2s, background .2s; outline: none; }
.form-input:focus { border-color: var(--accent); background: rgba(59,130,246,.06); }
.form-input::placeholder { color: var(--muted); }
.form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
.btn { display: inline-flex; align-items: center; gap: 7px; padding: 10px 20px; border-radius: 9px; font-family: var(--font-body); font-size: 14px; font-weight: 600; cursor: pointer; border: none; transition: all .18s; }
...
.btn-clear { background: rgba(100,116,139,.1); border: 1px solid rgba(100,116,139,.2); color: var(--muted); text-decoration: none; }
...
.data-table thead th { font-family: var(--font-mono); font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: var(--muted); padding: 0 16px 14px; text-align: left; border-bottom: 1px solid var(--card-border); white-space: nowrap; }
.data-table tbody td { padding: 13px 16px; border-bottom: 1px solid rgba(30,45,74,.5); vertical-align: middle; color: var(--text); }
...
.user-avatar { width: 32px; height: 32px; border-radius: 8px; background: linear-gradient(135deg, var(--accent), var(--accent2)); display: flex; align-items: center; justify-content: center; font-family: var(--font-mono); font-size: 11px; font-weight: 700; color: white; flex-shrink: 0; }
.pw-mask { font-family: var(--font-mono); font-size: 13px; letter-spacing: 2px; color: var(--muted); cursor: pointer; user-select: none; transition: color .15s; }
.pw-mask.shown { color: var(--accent2); letter-spacing: 0; }
.pw-toggle { margin-left: 6px; font-size: 13px; color: var(--muted); cursor: pointer; transition: color .15s; }
.pw-toggle:hover { color: var(--accent); }
.btn-hapus { display: inline-flex; align-items: center; gap: 5px; padding: 6px 12px; border-radius: 7px; background: rgba(239,68,68,.1); border: 1px solid rgba(239,68,68,.2); color: var(--danger); font-size: 12px; font-weight: 600; text-decoration: none; transition: all .18s; }
...
@media(max-width:768px){ body { padding: 20px 16px 40px; } .top-row { grid-template-columns: 1fr; } .form-row { grid-template-columns: 1fr; } }
</style>
</head>
<body>
        ...
        <div class="card-body">
            <form method="GET">
                ...
                <div class="search-row">
                    ...
                    <?php if($search): ?>
                    <a class="btn btn-clear" href="/web-radius/index.php?page=users">
                        <i class="bi bi-x"></i>
                    </a>
                    <?php endif; ?>
                </div>
        ...
        <div class="table-wrap">
            <table class="data-table">
                ...
                <tr>
                    ...
                    <td>
                        <a class="btn-hapus"
                           href="/web-radius/index.php?page=users&hapus=<?= $data['id'] ?>"
